/**
 * Buyer archetype options for Step 2 (controlled ids + display copy).
 * Single source of truth for the web wizard and deterministic PDF assembly.
 */
...
/**
 * Maps a Step 2 `customerArchetype` value to display copy for PDFs and summaries.
 * - Controlled ids (snake_case) resolve to the catalog `title` (e.g. `routine_upgrader` → "The Routine Upgrader").
 * - Uses `industry` first, then scans all industry lists (covers industry changes), then fallback generics.
 * - Legacy / free-text answers without underscores are returned unchanged.
 * - Unknown snake_case ids get title-cased words as a last resort.
 */
export function resolveBuyerArchetypeTitle(archetypeId: string, industry: string): string {
  const id = archetypeId.trim()
  if (!id) return ''

  const fromList = (list: BuyerArchetypeOption[] | undefined) => list?.find((o) => o.id === id)?.title

  const industryList = industryArchetypes[industry]
  const hitIndustry = fromList(industryList)
  if (hitIndustry) return hitIndustry

  for (const list of Object.values(industryArchetypes)) {
    const hit = fromList(list)
    if (hit) return hit
  }

  const hitFallback = fromList(fallbackArchetypes)
  if (hitFallback) return hitFallback

...
**Maps to W-track items:** W2 (Tier 1 and Tier 2 subsets for these two industries)

## How to use this document

All entries below are **additions** to existing leaves. The bank currently has 3 pairs per leaf. This document provides:

- Pairs **4–8** for Tier-1 leaves (≥8 target: WEBSITE all goals, EMAIL direct_sales and lead_gen)
- Pairs **4–6** for Tier-2 leaves (≥6 target: WEBSITE/EMAIL `retention`, EMAIL `audience_growth`)

Apply Wave A fixes first, then add these. Where Wave A changed a variant, it is noted so the integration agent knows the effective state of the leaf before appending.

---

## Wave C — Audience_growth creative framework

The structural problem in existing `audience_growth` copy: virtually every leaf uses the same two-sentence formula — `[Follow / subscribe for X, Y, Z]` + `[frequency promise]`. At depth this reads as generator output. The fix is not more pairs in the same shape — it's identifying distinct structural shapes per industry and distributing pairs across them so the pool reads as a diverse body of copy.

---

### Structural shapes — `trades_home` audience_growth

| Shape | Hook | Sample anchor |
|---|---|---|
| **Before/after** | Follow to see the transformation | "See what a good job looks like before you hire anyone" |
| **Local credibility** | Follow for work happening in your neighborhood | "Next post might be a job on your street" |
| **Pre-need tip** | Position as useful before the reader has a job | "Follow before you need us" |
| **Seasonal trigger** | Time the follow to the season they'll need this work | "Want a heads-up before peak season" |
| **Process transparency** | Show honest job-site reality | "The real photos and the honest story" |

Every `trades_home` audience_growth leaf must use at least three shapes.

---

### Structural shapes — `food_hospitality` audience_growth

| Shape | Hook | Sample anchor |
|---|---|---|
| **"What's on" access** | Follow to know the menu before deciding where to eat | "Post the daily menu every morning" |
| **Kitchen/sourcing** | Follow to see the people and sourcing behind the food | "Where it comes from, who makes it" |
| **Event/seasonal** | Follow to be first to know about events and seasonal dishes | "First to know, first to reserve" |
| **Regular connection** | Follow to stay close to a place between visits | "Stay close to the kitchen" |
| **Behind-the-scenes** | Follow for what doesn't appear on the menu | "The occasional recipe, the honest kitchen shot" |

Every `food_hospitality` audience_growth leaf must use at least four shapes.
...
### WEBSITE > `direct_sales` > `trades_home` > bold
...
### WEBSITE > `direct_sales` > `trades_home` > friendly
...
### WEBSITE > `direct_sales` > `trades_home` > professional
...
### WEBSITE > `lead_gen` > `trades_home` > bold
...
### WEBSITE > `lead_gen` > `trades_home` > friendly
...
### WEBSITE > `lead_gen` > `trades_home` > professional
...
### WEBSITE > `audience_growth` > `trades_home` > bold

_Existing pairs 1–3 unchanged._

**New pairs 4–8 (shapes: local, before/after, pre-need, process, seasonal):**
```
...
### WEBSITE > `audience_growth` > `trades_home` > friendly

_Existing pairs 1–3 unchanged._

**New pairs 4–8 (shapes: tip, process, seasonal, before/after+process, pre-need):**
```
...
### WEBSITE > `audience_growth` > `trades_home` > professional

_Pair 2 replaced by Wave A (A9). Treat as existing pair 2 when counting._

**New pairs 4–8:**
```
["Subscribe to receive seasonal maintenance guidance and completed project case studies.", "One publication per month. No advertising."],
...
["Subscribe to receive evidence-based maintenance planning guidance and project updates.", "Sent once a month. No third-party content."],
...
### WEBSITE > `retention` > `trades_home`

_Tier 2 target: ≥6 pairs. Existing pairs 1–3 unchanged._
...
### EMAIL > `direct_sales` > `trades_home` > bold
...
### EMAIL > `direct_sales` > `trades_home` > friendly
...
### EMAIL > `direct_sales` > `trades_home` > professional
...
["To receive a formal estimate, please reply with the project type and a brief scope description.", "We respond to all estimate requests within one business day."],
...
["Send project details by reply. We will provide a written estimate with a full scope breakdown.", "Response within one business day on all requests."],
```

---

### EMAIL > `lead_gen` > `trades_home` > bold
...
### EMAIL > `lead_gen` > `trades_home` > friendly

_Pair 3 replaced by Wave A (V1). Treat as existing pair 3._

**New pairs 4–8:**
```
["Not sure exactly what needs doing? That's what I'm here for.", "Tell me what you've seen and I'll help you figure out the job."],
["Hit reply with the situation, even if you're not sure how to describe it.", "I ask good questions. We'll get there together."],
...
### EMAIL > `lead_gen` > `trades_home` > professional
...
["Please reply with your project details and any available photos or documentation.", "We will follow up to confirm scope and arrange a visit if required."],
...
### EMAIL > `audience_growth` > `trades_home`
...
### WEBSITE > `direct_sales` > `food_hospitality` > bold
...
### WEBSITE > `direct_sales` > `food_hospitality` > friendly

_Pair 1 replaced by Wave A (A1). Pair 2 replaced by Wave A (A2)._
...
### WEBSITE > `direct_sales` > `food_hospitality` > professional
...
["Reserve your table or inquire about private dining.", "We accept reservations up to four weeks in advance."],
["View the menu and make your reservation online.", "Online ordering available for pickup and delivery."],
...
### WEBSITE > `lead_gen` > `food_hospitality` > bold
...
### WEBSITE > `lead_gen` > `food_hospitality` > friendly
...
### WEBSITE > `lead_gen` > `food_hospitality` > professional
...
### WEBSITE > `audience_growth` > `food_hospitality` > bold

_Existing pairs 1–3 unchanged. Shapes: W = "what's on", K = kitchen/sourcing, E = event/seasonal, R = regular connection, B = behind-the-scenes._
...
### WEBSITE > `audience_growth` > `food_hospitality` > friendly
...
["Sign up and we'll keep you in the loop.", "Special dinners, menu changes, and the stuff that never makes it onto the regular menu."],
...
_(shapes: K, E+B, K, W, K+B)_

---

### WEBSITE > `audience_growth` > `food_hospitality` > professional

_Pair 2 replaced by Wave A (A10). Treat as existing pair 2._

**New pairs 4–8:**
```
["Subscribe to receive advance notice of seasonal menu changes and private event availability.", "One publication per week, without exception."],
...
["Follow us for regular updates on our culinary calendar and upcoming events.", "Content published consistently each week."],
...
### WEBSITE > `retention` > `food_hospitality`

_Tier 2 target: ≥6 pairs. Pair 1 replaced by Wave A (A16)._
...
### EMAIL > `direct_sales` > `food_hospitality` > bold
...
### EMAIL > `direct_sales` > `food_hospitality` > friendly

_Pair 1 replaced by Wave A (A17)._
...
["Reply with how many, what day, and whether it's dinner or a bigger occasion.", "I'll confirm the reservation and let you know what's on that night."],
...
### EMAIL > `direct_sales` > `food_hospitality` > professional
...
["Reply with your reservation request and we will respond within one business day.", "Private dining and group bookings available upon inquiry."],
```

---

### EMAIL > `lead_gen` > `food_hospitality` > bold
...
### EMAIL > `lead_gen` > `food_hospitality` > friendly
...
### EMAIL > `lead_gen` > `food_hospitality` > professional
...
["Please reply with your event type, expected guest count, and any dietary requirements.", "We will respond with menu options and pricing within one business day."],
...
["Reply with your event requirements and we will prepare a tailored proposal.", "References and sample menus available upon request."],
...
### EMAIL > `audience_growth` > `food_hospitality`
...
### EMAIL > `retention` > `food_hospitality`
...
| Industry | Surface | Goal | Leaves | New pairs |
|---|---|---|---|---:|
| `trades_home` | WEBSITE | direct_sales (3 tones) | 3 × 5 | 15 |
| `trades_home` | WEBSITE | lead_gen (3 tones) | 3 × 5 | 15 |
| `trades_home` | WEBSITE | audience_growth (3 tones) | 3 × 5 | 15 |
| `trades_home` | WEBSITE | retention | 1 × 3 | 3 |
| `trades_home` | EMAIL | direct_sales (3 tones) | 3 × 5 | 15 |
| `trades_home` | EMAIL | lead_gen (3 tones) | 3 × 5 | 15 |
| `trades_home` | EMAIL | audience_growth | 1 × 3 | 3 |
| `trades_home` | EMAIL | retention | 1 × 3 | 3 |
| **Subtotal `trades_home`** | | | | **84** |
| `food_hospitality` | WEBSITE | direct_sales (3 tones) | 3 × 5 | 15 |
| `food_hospitality` | WEBSITE | lead_gen (3 tones) | 3 × 5 | 15 |
| `food_hospitality` | WEBSITE | audience_growth (3 tones) | 3 × 5 | 15 |
| `food_hospitality` | WEBSITE | retention | 1 × 3 | 3 |
| `food_hospitality` | EMAIL | direct_sales (3 tones) | 3 × 5 | 15 |
| `food_hospitality` | EMAIL | lead_gen (3 tones) | 3 × 5 | 15 |
| `food_hospitality` | EMAIL | audience_growth | 1 × 3 | 3 |
| `food_hospitality` | EMAIL | retention | 1 × 3 | 3 |
| **Subtotal `food_hospitality`** | | | | **84** |
| **WAVE B TOTAL** | | | | **168** |

---

*Apply after Wave A. Waves C onward cover the remaining five industries in the same pattern.*

...
## Narrator pairs in this wave

| Industry | Default narrator | Tuned leaf | Voice shift |
|---|---|---|---|
| `trades_home` | `solo_expert` | **`local_team`** | Crew dispatch and coverage → organized reliability replaces personal trust |
| `food_hospitality` | `local_team` | **`solo_maker`** | "I cook everything" → personal craft replaces team restaurant voice |
| `retail_maker` | `solo_maker` | **`product_led`** | Brand + collection voice → replaces single artisan "I make" |
| `health_wellness` | `solo_expert` | **`local_team`** | Multi-practitioner matching → organized breadth replaces personal expertise |
| `creative_pro` | `local_team` | **`solo_expert`** | Single creator with limited slots → personal selectivity replaces studio voice |
| `professional_svc` | `local_team` | **`solo_expert`** | "I practice independently" → personal accountability replaces firm voice |

**Quality bar:** each pair must pass the read-aloud test as something the specific narrator type would actually say. A pop-up chef doesn't say "our kitchen." A solo accountant doesn't say "our team will follow up."

---

---

## 1. `trades_home` × `local_team`

**Voice:** An organized crew — 4+ vans, a dispatch function, coverage across a region. Trust signals shift from personal competence ("I'll come take a look") to organizational reliability ("our team covers the whole area", "we dispatch within the week"). The team can talk about scheduling systems, availability windows, response SLAs.

**Key vocabulary:** "our team", "we'll send someone out", "our crew", "dispatch", "we cover", "our estimators", "within one business day".
...
### WEBSITE > `direct_sales` > `trades_home` × `local_team` > friendly
...
### WEBSITE > `direct_sales` > `trades_home` × `local_team` > professional
...
### WEBSITE > `lead_gen` > `trades_home` × `local_team` > bold
...
### WEBSITE > `lead_gen` > `trades_home` × `local_team` > friendly
...
### WEBSITE > `lead_gen` > `trades_home` × `local_team` > professional
...
### EMAIL > `direct_sales` > `trades_home` × `local_team` > bold
...
### EMAIL > `direct_sales` > `trades_home` × `local_team` > friendly
...
### EMAIL > `direct_sales` > `trades_home` × `local_team` > professional
...
### EMAIL > `lead_gen` > `trades_home` × `local_team` > bold
...
### EMAIL > `lead_gen` > `trades_home` × `local_team` > friendly
...
### EMAIL > `lead_gen` > `trades_home` × `local_team` > professional
...
### SOCIAL > `direct_sales` > `trades_home` × `local_team` > bold
...
### SOCIAL > `direct_sales` > `trades_home` × `local_team` > friendly
...
### SOCIAL > `lead_gen` > `trades_home` × `local_team` > friendly
...
## 2. `food_hospitality` × `solo_maker`

**Voice:** A pop-up chef, private dinner host, solo caterer, cottage baker, one-person food truck. The meal is inseparable from the person making it. Trust is built on access to a specific person's hands and recipes — not a kitchen team. Intimate scale is the product.

**Key vocabulary:** "I cook", "I make", "my menu", "my pop-up", "DM me", "limited seats", "made by me", "the next date", personal schedule, "book a seat at my table", specific group size constraints.
...
### WEBSITE > `direct_sales` > `food_hospitality` × `solo_maker` > friendly
...
### WEBSITE > `direct_sales` > `food_hospitality` × `solo_maker` > professional
...
### WEBSITE > `lead_gen` > `food_hospitality` × `solo_maker` > friendly
...
### WEBSITE > `lead_gen` > `food_hospitality` × `solo_maker` > professional
...
### EMAIL > `direct_sales` > `food_hospitality` × `solo_maker` > bold
...
### EMAIL > `direct_sales` > `food_hospitality` × `solo_maker` > friendly
...
### EMAIL > `direct_sales` > `food_hospitality` × `solo_maker` > professional
...
### EMAIL > `lead_gen` > `food_hospitality` × `solo_maker` > bold
...
### EMAIL > `lead_gen` > `food_hospitality` × `solo_maker` > friendly
...
### EMAIL > `lead_gen` > `food_hospitality` × `solo_maker` > professional
...
### SOCIAL > `direct_sales` > `food_hospitality` × `solo_maker` > bold
...
### SOCIAL > `direct_sales` > `food_hospitality` × `solo_maker` > friendly
...
### SOCIAL > `lead_gen` > `food_hospitality` × `solo_maker` > bold
...
### SOCIAL > `lead_gen` > `food_hospitality` × `solo_maker` > friendly
...
## 3. `retail_maker` × `product_led`

**Voice:** A maker who has grown into a brand. Still handmade but now operates as a multi-product label — collections, seasonal drops, editorial presentation. The brand is bigger than one person. "We design and produce" replaces "I make."

**Key vocabulary:** "shop the collection", "our latest drop", "the [collection]", "we design and make", "our studio", "seasonal release", brand-tone vocabulary, product categories.

---

### WEBSITE > `direct_sales` > `retail_maker` × `product_led` > bold
...
### WEBSITE > `direct_sales` > `retail_maker` × `product_led` > friendly
...
### WEBSITE > `direct_sales` > `retail_maker` × `product_led` > professional
...
### WEBSITE > `lead_gen` > `retail_maker` × `product_led` > bold
...
### WEBSITE > `lead_gen` > `retail_maker` × `product_led` > friendly
...
### WEBSITE > `lead_gen` > `retail_maker` × `product_led` > professional
...
### EMAIL > `direct_sales` > `retail_maker` × `product_led` > bold
...
### EMAIL > `direct_sales` > `retail_maker` × `product_led` > friendly
...
### EMAIL > `direct_sales` > `retail_maker` × `product_led` > professional
...
### EMAIL > `lead_gen` > `retail_maker` × `product_led` > bold
...
### EMAIL > `lead_gen` > `retail_maker` × `product_led` > friendly
...
### EMAIL > `lead_gen` > `retail_maker` × `product_led` > professional
...
### SOCIAL > `direct_sales` > `retail_maker` × `product_led` > bold
...
### SOCIAL > `direct_sales` > `retail_maker` × `product_led` > friendly
...
### SOCIAL > `lead_gen` > `retail_maker` × `product_led` > bold
...
### SOCIAL > `lead_gen` > `retail_maker` × `product_led` > friendly
...
## 4. `health_wellness` × `local_team`

**Voice:** A multi-practitioner clinic, yoga studio with multiple teachers, group therapy practice, physical therapy center. The value proposition is breadth of expertise and matching — "we'll put you with the right person." Scheduling is more flexible because there are more people available.

**Key vocabulary:** "our team", "our practitioners", "we'll match you", "multiple disciplines", "the right practitioner", "we cover", breadth signals, team-based availability.

---

### WEBSITE > `direct_sales` > `health_wellness` × `local_team` > bold
...
### WEBSITE > `direct_sales` > `health_wellness` × `local_team` > friendly
...
### WEBSITE > `direct_sales` > `health_wellness` × `local_team` > professional
...
### WEBSITE > `lead_gen` > `health_wellness` × `local_team` > bold
...
### WEBSITE > `lead_gen` > `health_wellness` × `local_team` > friendly
...
### WEBSITE > `lead_gen` > `health_wellness` × `local_team` > professional
...
### EMAIL > `direct_sales` > `health_wellness` × `local_team` > bold
...
### EMAIL > `direct_sales` > `health_wellness` × `local_team` > friendly
...
### EMAIL > `direct_sales` > `health_wellness` × `local_team` > professional
...
### EMAIL > `lead_gen` > `health_wellness` × `local_team` > bold
...
### EMAIL > `lead_gen` > `health_wellness` × `local_team` > friendly
...
### EMAIL > `lead_gen` > `health_wellness` × `local_team` > professional
...
### SOCIAL > `direct_sales` > `health_wellness` × `local_team` > bold
...
### SOCIAL > `direct_sales` > `health_wellness` × `local_team` > friendly
...
### SOCIAL > `lead_gen` > `health_wellness` × `local_team` > bold
...
### SOCIAL > `lead_gen` > `health_wellness` × `local_team` > friendly
...
## 5. `creative_pro` × `solo_expert`

**Voice:** A solo photographer, independent brand designer, freelance copywriter, one-person motion studio. The person *is* the product — their taste, their process, their attention. Scarcity is structural ("I take on a limited number of clients"). Every client gets the actual expert, not a team member.

**Key vocabulary:** "I take on", "my work", "my process", "I work with a small number of clients", "DM me directly", personal portfolio ownership, availability as scarce, "I give every project my full attention", "I reply personally".

---

### WEBSITE > `direct_sales` > `creative_pro` × `solo_expert` > bold
...
### WEBSITE > `direct_sales` > `creative_pro` × `solo_expert` > friendly
...
### WEBSITE > `direct_sales` > `creative_pro` × `solo_expert` > professional
...
### WEBSITE > `lead_gen` > `creative_pro` × `solo_expert` > friendly
...
### WEBSITE > `lead_gen` > `creative_pro` × `solo_expert` > professional
...
### EMAIL > `direct_sales` > `creative_pro` × `solo_expert` > bold
...
### EMAIL > `direct_sales` > `creative_pro` × `solo_expert` > friendly
...
### EMAIL > `direct_sales` > `creative_pro` × `solo_expert` > professional
...
### EMAIL > `lead_gen` > `creative_pro` × `solo_expert` > bold
...
### EMAIL > `lead_gen` > `creative_pro` × `solo_expert` > friendly
...
### EMAIL > `lead_gen` > `creative_pro` × `solo_expert` > professional
...
### SOCIAL > `direct_sales` > `creative_pro` × `solo_expert` > bold
...
### SOCIAL > `direct_sales` > `creative_pro` × `solo_expert` > friendly
...
### SOCIAL > `lead_gen` > `creative_pro` × `solo_expert` > bold
...
### SOCIAL > `lead_gen` > `creative_pro` × `solo_expert` > friendly
...
## 6. `professional_svc` × `solo_expert`

**Voice:** An independent attorney, solo accountant, one-person financial advisor, independent estate planner. "I practice independently" — the practitioner's personal attention is the differentiator. No hand-offs, no associates, no firm overhead. You get the expert you called.

**Key vocabulary:** "I practice independently", "I handle every file personally", "I work with a small number of clients", "you deal with me directly", "no hand-offs", personal accountability, "I respond personally".

---

### WEBSITE > `direct_sales` > `professional_svc` × `solo_expert` > bold
...
### WEBSITE > `direct_sales` > `professional_svc` × `solo_expert` > friendly
...
### WEBSITE > `direct_sales` > `professional_svc` × `solo_expert` > professional
...
### WEBSITE > `lead_gen` > `professional_svc` × `solo_expert` > bold
...
### WEBSITE > `lead_gen` > `professional_svc` × `solo_expert` > friendly
...
### WEBSITE > `lead_gen` > `professional_svc` × `solo_expert` > professional
...
### EMAIL > `direct_sales` > `professional_svc` × `solo_expert` > bold
...
### EMAIL > `direct_sales` > `professional_svc` × `solo_expert` > friendly
...
### EMAIL > `direct_sales` > `professional_svc` × `solo_expert` > professional
...
### EMAIL > `lead_gen` > `professional_svc` × `solo_expert` > bold
...
### EMAIL > `lead_gen` > `professional_svc` × `solo_expert` > friendly
...
### EMAIL > `lead_gen` > `professional_svc` × `solo_expert` > professional
...
### SOCIAL > `direct_sales` > `professional_svc` × `solo_expert` > bold
...
### SOCIAL > `direct_sales` > `professional_svc` × `solo_expert` > friendly
...
### SOCIAL > `lead_gen` > `professional_svc` × `solo_expert` > bold
...
### SOCIAL > `lead_gen` > `professional_svc` × `solo_expert` > friendly
...
| Narrator pair | Leaves | Pairs/leaf | Subtotal |
|---|---:|---:|---:|
| `trades_home` × `local_team` | 16 | 5 | 80 |
| `food_hospitality` × `solo_maker` | 16 | 5 | 80 |
| `retail_maker` × `product_led` | 16 | 5 | 80 |
| `health_wellness` × `local_team` | 16 | 5 | 80 |
| `creative_pro` × `solo_expert` | 16 | 5 | 80 |
| `professional_svc` × `solo_expert` | 16 | 5 | 80 |
| **WAVE E TOTAL** | **96** | — | **480** |

**Leaves per narrator pair:** WEBSITE × (direct_sales + lead_gen) × 3 tones + EMAIL × (direct_sales + lead_gen) × 3 tones + SOCIAL × (direct_sales + lead_gen) × 2 tones = 6 + 6 + 4 = 16 leaves.

---

_Wave F covers marketplace gap closures (§4B: Marketplace × `audience_growth` for `retail_maker` and Marketplace × `retention` for `retail_maker` + `trades_home`) and Tier-2 depth on the remaining surfaces (Google Business `direct_sales`, Social Casual `audience_growth`, WEBSITE/EMAIL `retention`)._

...
| CSP — brand summary one-liner | Sonnet 4.5 | 1 | ≤ 16 per variant (3 variants) | `{ variants: { angle, text, fieldsCited }[], fieldsCited }` | yes | no | **`ai_enhanced`** — scaffold-and-refine; see [`CONTENT_STARTER_PACK.md`](../specs/CONTENT_STARTER_PACK.md) |
| CSP — brand summary elevator | Sonnet 4.5 | 1 | ≤ 60 | `{ text, fieldsCited }` | yes | no | **`ai_enhanced`** |
| CSP — brand summary paragraph | Sonnet 4.5 | 1 | ≤ 120 | `{ text, fieldsCited }` | yes | no | **`ai_enhanced`** |
| CSP — homepage directions | Sonnet 4.5 | 1 | ≤ 10 headline + ≤ 18 subhead per route (2–3 routes) | `{ routes: { headline, subhead, fieldsCited }[] }` | per route | no | **`ai_enhanced`** — refine wording only; route strategy fixed in scaffold |
| CSP — bio short | Sonnet 4.5 | 1 | ≤ 30 | `{ text, fieldsCited }` | yes | no | **`ai_enhanced`** — profile-field paste-ready |
| CSP — bio long / About | Sonnet 4.5 | 1 | ≤ 60 | `{ text, fieldsCited }` | yes | no | **`ai_only`** (minimal stub on failure) |
| CSP — caption starters | Sonnet 4.5 | 1 | 6–8 × ≤ 25w (cap 4 when no `voiceSamples`) | `{ starters: { text, fieldsCited }[] }` | per starter | no | **`ai_only`** (2 industry stubs on failure) |
| CSP — content pillars | Sonnet 4.5 | 1 | 4–5 pillars × (oneLine ≤ 30w + 2 prompts ≤ 25w) | `{ pillars: { name, oneLine, prompts: string[], fieldsCited }[] }` | per pillar | no | **Hybrid** — pillar **names** locked from narrator profile; AI fills lines + questions only |
...
#### §12.9.2 Content Starter Pack (Sonnet) — ~8 calls (hybrid)

**Canonical contract:** [`docs/specs/CONTENT_STARTER_PACK.md`](../specs/CONTENT_STARTER_PACK.md) — supersedes the earlier all-`ai_only` / blank-slate CSP model.

**Purpose.** Paste-ready applied copy for the buyer to use immediately — three one-liner angles, elevator pitch, brand paragraph, homepage messaging routes, two bio lengths, caption starters, and content pillars. CSP page 2's CTA section is rendered from `voice.ctaVariations` (§12.9.3) rather than generated by an independent CSP CTA prompt — see the render-alias note below and [`OUTPUT_TRANSLATION_SPEC.md`](../../OUTPUT_TRANSLATION_SPEC.md) §10A.6A.1 for the locked single-source contract.

**Generation pattern.** CSP uses the same **scaffold-and-refine** model as Core `ai_enhanced` rewrites for rows marked `ai_enhanced` in the CSP spec. Deterministic builders in `contentStarterScaffolds.ts` produce the scaffold; the task prompt includes verbatim scaffold text plus "rewrite in voice, do not add uncited facts." `ai_only` rows (long bio, captions) receive minimal stubs on failure. Hybrid pillars: names are fixed in code; AI only fills `oneLine` + `prompts[]`.

**Task prompt templates** (each opens with the locked persona-recall line; `ai_enhanced` rows also include the deterministic scaffold block):

- One-liner (3 variants): *"… rewrite each deterministic seed line in their voice. Keep each variant 8–16 words and its angle (transformation / audience / differentiator). Do not add uncited claims."*
- Elevator: *"… expand the deterministic scaffold to ~60 words (2–3 sentences) in their voice. Stay within cited intake."*
- Paragraph: *"… expand the deterministic scaffold to ~120 words (one paragraph: who, for whom, what changes) in their voice."*
- Homepage directions: *"… refine wording of each route's headline and subhead in their voice. Do **not** change route strategy or add new promises."*
- Bio short: *"… rewrite the deterministic scaffold to 15–30 words, 1–3 lines, suitable for a social-profile bio field. Emoji only if `tonePreset === 'friendly'`."*
- Bio long / About: *"… write a 40–60 word About-page opener in their voice, grounded in story + values when present."*
- Caption starters: *"… write 6–8 caption-starter phrases (cap 4 when `voiceSamples` is empty). Each ≤ 25 words. Vary openers; no two start the same way."*
- Content pillars: *"… for each **fixed** pillar name in the scaffold, write a one-line value statement (≤ 30 words) and exactly 2 starter questions (≤ 25 words each). Do **not** rename pillars."*

**CTAs — render alias, not a prompt call.** The CSP page 2 CTA section renders the structured output of `voice.ctaVariations` (§12.9.3). No separate CSP CTA prompt exists; no `csp.ctas.v1.ts` file is created. Variations use locked intents: `more_direct`, `quieter`, `more_inviting`, `more_confident`.

**Schemas.** As in the catalog table and [`CONTENT_STARTER_PACK.md`](../specs/CONTENT_STARTER_PACK.md) § Structured output schemas. Every output field — even sub-array elements — carries its own `fieldsCited`.

**Banned-vocab additions.** None beyond the inherited list for the 8 CSP prompt calls above. The CTA variation walker chain (banned-vocab + `CTA_COPY_RULES.md` avoid list + em-dash-as-period rule) applies once on the `voice.ctaVariations` output and the result is trusted by both downstream renderers.

**Failure mode.** Per section: walker failure or API failure → ship the **deterministic scaffold** for that section (`ai_enhanced` / hybrid) or the **minimal stub** (`ai_only`). The CSP PDF **always assembles** — degraded CSP beats a missing PDF. CTA failure is owned by §12.9.3 — when that call fails for a surface, both CSP page 2 and Voice Playbook page 3 ship the deterministic anchor CTA only for that surface.

...
**Status:** Sprint-prep for Pro-A. Specifies how the ~26 per-call AI invocations in [`AI_INTEGRATION_PLAYBOOK.md`](./AI_INTEGRATION_PLAYBOOK.md) assemble into a complete Pro kit fulfillment lifecycle. The playbook owns the **per-call** contract; this doc owns the **per-kit** contract.
...
flowchart TD
  ...
  FanOut --> CSP["~8 CSP sections<br/>(Sonnet 4.5)"]

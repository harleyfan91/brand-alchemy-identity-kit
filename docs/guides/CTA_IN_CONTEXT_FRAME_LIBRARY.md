...
10. **Persuasion trust guard** — CTA copy can be specific and conversion-oriented, but do not fabricate urgency, stock scarcity, or social proof in reader-visible lines. Keep claims verifiable and deterministic.
...
5. **Model** — [`composeCtaSurfaceBlocks`](../../packages/generation/src/deterministic/brandIdentityGuideModel.ts) attaches `presentation` (`frameId` plus surface-specific fields). Social nested **label** uses the same touchpoint labels as `platformSummary`. CTA text is composed through deterministic strategy routing (`primaryGoal` -> surface/touchpoint family -> operating/offer context -> risk/voice modulation) before frame rendering.

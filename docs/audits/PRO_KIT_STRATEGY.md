...
- **Section walker** (banned vocab, em-dash budget, no fabricated metrics) extends from `core-pdfs.test.ts` to AI output.
...
|---|---|---|
| **Pro-0** (Core prereq, parallel) | CTA bank audit per §7.5 — expand prescriptive phrase banks, add path-class variety, retest folio 05 across all 8 personas | All 8 personas produce non-templated, on-brand CTAs |
...
| New doc `docs/research/STRATEGIST_MEMO_PROMPTING.md` | **Recommended.** Section-by-section system prompts, example outputs across fixtures, banned vocab list, length budgets, claim-citation requirement, **messaging-hierarchy proof-point citation discipline**, **brand-narrative selector logic (Problem Story vs Manifesto substance thresholds)**. |

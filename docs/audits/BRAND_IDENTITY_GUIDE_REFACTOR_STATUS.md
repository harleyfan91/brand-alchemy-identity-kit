...
- **Folio 05 surface CTAs — sequencing (still open):** Prioritize **CTA line composition** (`linesForSurface`, `composeCtaSurfaceBlocks`, de-dupe, caps, hooks) **before** new **`pickCtaFrameId`** routes (`email_image_v1` defaults, `social_carousel_v1` primary routing, extra website interiors). **Layout / slot / Yoga fixes** shipped ahead of that copy pass — see **Folio 05 — Examples** audit table above; not a substitute for the composition work.

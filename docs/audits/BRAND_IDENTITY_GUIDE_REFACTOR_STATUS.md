...
- **Folio 05 surface CTAs — sequencing (v3 industry + paste-ready shipped):** CTA lines are composed in `ctaSurfacePhrases.ts` (industry group from `step1.industry`, `tonePreset`, `CtaSurfaceActionMode`, social platform family) and selected deterministically in `composeSurfaceCtaLines` / `composeCtaSurfaceBlocks`, with `examples.ctaTemplates` using the same signals via `buildCtaTemplateLines`. Policy: `OUTPUT_TRANSLATION_SPEC.md` §10A.6A. Remaining open item is **full offer-type taxonomy depth** and optional frame-route expansions, not baseline routing.
...
- **Folio 05 — slot model (shipped v1):** `frameId → slot class` ([`slotClass.ts`](../../packages/generation/src/pdf/ctaFrames/slotClass.ts)), folio template chooser ([`ctaFolioTemplate.ts`](../../packages/generation/src/pdf/ctaFrames/ctaFolioTemplate.ts)), and Examples **row/stack layout** in [`CoreKitDocuments.tsx`](../../packages/generation/src/pdf/CoreKitDocuments.tsx). Shared geometry in [`socialFeedLayout.ts`](../../packages/generation/src/pdf/ctaFrames/socialFeedLayout.ts) (including **mobile_tall** linear scale). Normative tables: [Normative slot geometry](../guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md#normative-slot-geometry-v1).
- **Folio 04 Voice:** **bottom band** below the two-column grid — **deferred** until Quick Start refactor ([`QUICK_START_REFACTOR.md`](./QUICK_START_REFACTOR.md)); likely becomes a one-line pointer to Quick Start Week *n*, not standalone instructional copy.
...
- **Narrow column:**
  ...
  - **Fallback branch** — **What it stands for** (`positioning.standsForLine`) from `composePersonalityStandsFor` when triplet confidence fails or sparse mode is active. Sparse mode now preserves a short intake-derived mission/motivation line when available, while still dropping narrator fallback.
- **Wide column:**
  - **Focus lead** (`positioning.focusLead`) as the top paragraph.
  - Exactly one framing body below the focus lead (gradient `GuidePersonalityQuotePanelWithRadial`):
    - **`storyNote`** when **`composePersonalityStoryQuote`** in [`personalityStoryQuote.ts`](../../packages/generation/src/deterministic/personalityStoryQuote.ts) returns a qualifying paragraph (causal arc, name anchor, semicolon join on observation→mission path, `I`/`we` from narrator for bare mission text; industry + professional-formality gates suppress the casual template for restrained kits). **Not** derived by slicing the Brief “Brand story angle” body.
    - Otherwise **`oneLine`** (mirror of `summary.oneLine`). When `storyNote` is present, `oneLine` is omitted on this spread.
  - **Brand behavior** (`positioning.behavior`) — three short behavior lines (**Shows up as**, **Avoids**, **Earns trust by**) composed from tone, industry, Step 2 pain/outcome, and Step 7 competitor signals. This gives the page a competitor-guide-style value object without raw intake lists.
  - **One trust cue** rendered inline at the bottom (no separate side rail), selected by `selectPositioningTrustCue`: `differentiator` (when substantive) > `collaborator` (when `emphasis === 'handoff'`, labeled *For someone helping you*) > plain `generic` fallback. The trust body now accepts a typed context object and can be sharpened by Step 2 outcomes/pain, Step 7 competitors, and compliance-sensitive industry routing.
- **Layout fill:** the narrow column is an **open editorial rail** (mini-header **Brand heart**, `GuideOpenModule` blocks, hairline dividers — `guidePersonalityBrandHeartRoot` / `guidePersonalityHeartSectionRule`); the wide column uses a **left-accent quote rail** (`guidePersonalityQuotePanel`, no gray fill), rule-separated **Brand behavior** rows (`guidePersonalityBehaviorFirstRow` / `guidePersonalityBehaviorRuleRow`), then the trust cue with a closing top rule (`guidePersonalityTrustClose`). No stock imagery or invented brand assets were added.
...
- Continue tuning exact phrasing for `positioning.behavior` and trust-cue enrichment as more persona fixtures are reviewed.
...
**Audit — plan vs shipped (layout / PDF, not copy):** The refactor plan’s **sequencing** said: tighten **CTA line composition** in the model (`linesForSurface`, `composeCtaSurfaceBlocks`, …) **before** extending **`pickCtaFrameId` / new shells**. In code we **did not** add new social `frameId`s or persona-specific heuristics for QA fixtures. We **did** ship **cross-cutting PDF/layout fixes** that the plan/status docs had not yet spelled out as “done”:

| Area | Plan / prior status wording | What shipped instead |
|------|------------------------------|-------------------------|
| **Slot class for LinkedIn** | `social_link_preview_v1` documented as **`compact_chip`** | Reclassified **`desktop_wide`** so pairs like website + LinkedIn **`stack_vertical`** at hero width instead of **`desktop_compact_row`** (168 pt rail). |
| **Folio 05 column split** | “~82%” CTA column, margin-only gutter | **~80.5%** CTA column (floor for `desktop_compact_row`), **12 pt vertical rule** between CTA and editorial, **`alignItems: 'stretch'`** on stack/single wrappers so **`width: '100%'`** shells get a real width in Yoga. |
| **Story / reel scale** | Normative table listed older **96 / 94** pt story widths | **Uniform ~7.5% scale** in `socialFeedLayout.ts` (`MOBILE_TALL_SHELL_SCALE`) — aspect ratios preserved; `mobile_desktop_row` mobile column width tracks **`SOCIAL_STORY_CARD_WIDTH_PT`**. |
| **Row templates + react-pdf** | Templates described as `flex: 1` two-mobile | **`two_mobile_row`**: fixed half-cell widths + **`skipWrapLock`** on nested `GuideOpenModule` (avoid `wrap={false}` collapsing row siblings). **`SocialHeader`** progress bar: **`width: '100%'`** + **`maxWidth`** so skeleton lines do not outgrow narrow grid shells. |
| **`cardAlignSelf` (guide)** | Not in original slot doc | Optional prop on frames + guide passes **`flex-start`** so shells anchor **bottom-left** in the CTA column (explorer unchanged). |

**Doc sync:** [`CTA_IN_CONTEXT_FRAME_LIBRARY.md`](../guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md) (folio 05 table, slot class table, link-preview footprint) and **OUTPUT_TRANSLATION_SPEC §10A.6A** updated to match the table above. **Copy composition** for folio 05 now routes through industry groups + `tonePreset` + surface/action modes (`ctaSurfacePhrases.ts`, §10A.6A v3); frame routing (`pickCtaFrameId`) remains the next sequencing frontier when product expands shells.
...
- **Sample lines** row at top (folio 05), with **Do / avoid** moved to **folio 04 (Voice)** so Examples can use a full-width CTA band + L-shaped top row within the six-page contract.
- **`examples.ctaSurfaces`** when touchpoints are selected: nested modules under **Calls to action** in a **full-width** band (surface order + cap from `pickSurfaces` / `maxCtaSurfaces`), each with up to two paste lines and optional `presentation.frameId` for a **vector in-context shell** (see [`CTA_IN_CONTEXT_FRAME_LIBRARY.md`](../guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md), including [Folio 05 layout budget](../guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md#folio-05-layout-budget-brand-identity-guide)). When touchpoints are empty, **`examples.ctaTemplates`** still supplies the CTA block in that band.
- **CTA templates** (`examples.ctaTemplates`) — 2–3 copy-ready CTA lines shaped by `primaryGoal` (`direct_sales` / `lead_gen` / `audience_growth` / `retention`), rendered in the full-width CTA band when the surface stack is not used. Replaces the previous abstract CTA column on Voice.
- Fixed **split rail** on Examples: before/after **only when pairs qualify under §10A.8** (no Do / avoid on folio 05).
...
- **Sequence:** **CTA line composition** for paste-ready surfaces/templates is now industry- and tone-aware (`ctaSurfacePhrases.ts`, `composeSurfaceCtaLines`). Next sequencing step is expanding **frame selection** (`pickCtaFrameId` — e.g. hero-image email, carousel-first social, extra website shells) when product wants richer in-context shells. Vector assets in `packages/generation/src/pdf/ctaFrames/` remain build-ready.
- **Layout:** Resolve folio 05 **vertical overflow** when multiple tall shells stack (see [layout budget](../guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md#folio-05-layout-budget-brand-identity-guide)); options include break-friendly wrapping, continuation layout, compact variants, or stricter caps — any change must preserve the **six-page** `core-pdfs` contract unless product explicitly expands the kit.
...
Last updated: 2026-04-25 (folio 05 **slot geometry + templates** implemented: `socialFeedLayout.ts` tightened, [`slotClass.ts`](../../packages/generation/src/pdf/ctaFrames/slotClass.ts) / [`ctaFolioTemplate.ts`](../../packages/generation/src/pdf/ctaFrames/ctaFolioTemplate.ts), Examples CTA region layout in [`CoreKitDocuments.tsx`](../../packages/generation/src/pdf/CoreKitDocuments.tsx); playbook normative tables + unit tests.)
...
Last updated: 2026-05-07 (folio **03** gradient quote: `storyNote` is now composed by **`composePersonalityStoryQuote`** — Brief story-block slicing retired; observation paths use **semicolon** between context and mission commitment, **narrator `I`/`we`** defaults, and **industry** (`legal_professional_services` / `finance` / `health_wellness`) + **professional + high formality** gates suppress the casual causal template so Core/restrained kits are not over-casualized. Core path still omits `storyNote` when Pro-only story fields are absent (gradient shows `oneLine`). Spec: `OUTPUT_TRANSLATION_SPEC.md` **§10A.7.1** + **§10A.13** item 2b; tests: `personalityStoryQuote.test.ts` + `core-pdfs.test.ts`.)
...
Last updated: 2026-04-28 (folio **03** Personality enrichment: widened `selectPositioningTrustCue` to a typed context object carrying Step 2 audience/pain/outcome, Step 7 competitors/differentiation, industry, density, collaborator, and feel signals. Trust cue copy now stays one cue but can use outcome fit, audience pain, category clarity, and compliance-softened phrasing instead of generic *trustworthy before impressive* copy. Added `positioning.behavior` (**Shows up as**, **Avoids**, **Earns trust by**) as a compact brand-behavior value object. Sparse mode still drops the full triplet, but now preserves a short intake-derived `standsForLine` when available and drops narrator fallback. PDF folio 03 now renders a tinted **Brand heart** card, larger story/promise quote panel, behavior card, and trust stack to improve page occupancy without stock imagery or a seventh page.)

Last updated: 2026-04-27 (folio **03** editorial reflow: retired tinted **Brand heart** and behavior `GuideCard` chrome on Personality in favor of an open narrow rail with hairlines between Vision / Mission / Promise (and after Feel when present), a transparent quote panel with a left accent rule, rule-separated behavior rows, and a hairline-led trust close — aligned with folios 01–02a/b whitespace + hairline rhythm. Spec §10A.7 / §10A.13 and `core-pdfs.test.ts` folio 03 source assertions updated accordingly.)
...
Last updated: 2026-04-30 (folio **05** CTA diversification pass: added deterministic variant selection to `brandIdentityGuideModel.ts` using a stable signal fingerprint (goal/surface/touchpoint family + operating/offer + stage/industry/tone + pain/outcome hints) so same input stays reproducible while different fixtures spread across larger phrase matrices. Expanded per-surface/per-action copy banks (website/email/directory/marketplace/social, casual/pro) with human-tone cadence variants while preserving policy guardrails and presentation/frame contracts in `CoreKitDocuments.tsx`. `core-pdfs.test.ts` now includes anti-homogeneity checks for cross-fixture CTA diversity and channel-surface idiom coverage in addition to determinism/banned-vocabulary safeguards; full generation suite remains green (308/308). Fixture cohort render run completed for `default`, `coffee-founder`, `established-pro`, `community-org`, `cta-mixed`, and `lean-core`; six-page stability and expected folio 05 layout-template routing remain intact.)

Last updated: 2026-05-01 (folio **05** industry-aware paste-ready CTAs: canonical `step1.industry` maps to coarse **industry groups** in [`ctaIndustryGroups.ts`](../../packages/generation/src/deterministic/ctaIndustryGroups.ts); phrase banks and template fallbacks live in [`ctaSurfacePhrases.ts`](../../packages/generation/src/deterministic/ctaSurfacePhrases.ts) with `CtaSurfaceActionMode`, `tonePreset` (`voiceTone`), social platform family (`socialTone`), and `riskProfile` gating. `composeCtaSurfaceBlocks` / `composeSurfaceCtaLines` / `composeCtaTemplates` thread the same signals; **OUTPUT_TRANSLATION_SPEC** §10A.6A documents routing. `core-pdfs.test.ts` adds explicit industry-swap differentiation for website `lead_gen` plus dedicated `ctaIndustryGroups.test.ts`.)

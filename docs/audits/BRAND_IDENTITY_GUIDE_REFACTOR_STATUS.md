...
- **Layout fill:** the narrow column now renders as a tinted **Brand heart** card; the wide column uses a larger story/promise quote panel plus the behavior card and trust cue stack (`guidePersonalityQuotePanel`, `guidePersonalityWideStack`, `guidePersonalityBehaviorRow`). No stock imagery or invented brand assets were added.

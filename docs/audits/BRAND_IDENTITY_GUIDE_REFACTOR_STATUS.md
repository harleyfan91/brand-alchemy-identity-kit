...
- **Folio 05 surface CTAs — sequencing (v2 diversification shipped):** CTA composition now runs through deterministic layers (`primaryGoal` -> surface/touchpoint family -> operating/offer context -> industry/stage risk trim -> voice modulation -> stable variant selection) in `composeCtaSurfaceBlocks` / `linesForSurface`, with policy guardrails in `OUTPUT_TRANSLATION_SPEC.md` §10A.5 + §10A.6A. Remaining open item is **offer taxonomy depth** and future frame-route expansions, not baseline composition/variation architecture.
...
**Doc sync:** [`CTA_IN_CONTEXT_FRAME_LIBRARY.md`](../guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md) (folio 05 table, slot class table, link-preview footprint) and **OUTPUT_TRANSLATION_SPEC §10A.6A** updated to match the table above. **Copy composition** (`linesForSurface`, tone × goal specificity) remains the **open** sequencing item from the plan.

...
**Layout budget (folio 05):** When `examples.ctaSurfaces` is non-empty, the PDF places **up to two or three** nested surface modules (each may render a full vector shell) **above** the split rail on the Examples spread, using **deterministic row or vertical templates** driven by slot classes (see [`docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md`](docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md#folio-05-layout-budget-brand-identity-guide)). Normative geometry and `frameId → slot class` mapping: [`docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md`](docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md#normative-slot-geometry-v1). **Minimum** in-context shells in that path is **0** if all surfaces drop for empty lines after de-dupe (edge case); **design for 1–3** nested shells capped by `contentDensityBias`.

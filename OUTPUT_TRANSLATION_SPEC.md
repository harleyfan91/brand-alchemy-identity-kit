...
**In-context frames (v0):** when `examples.ctaSurfaces[].presentation.frameId` is set, folio 05 renders a **vector post shell** (avatar, meta line, media block, **one caption block** that joins up to two composed strings with a single space so it reads as one paste field, generic action icons) instead of a bare numbered list for that surface. Social now ships four frame ids: `social_feed_v1`, `social_story_v1`, `social_reel_cover_v1`, `social_grid_photo_v1`; other non-social surfaces keep the list until additional `frameId` values exist. For social, `presentation.platformSummary` matches the nested module label (which channels you picked), `presentation.socialSurfaceFamily` captures feed/story/reel/grid mapping, and `presentation.socialFeedVariant` applies only to `social_feed_v1` geometry. Fixed media dimensions are defined in `socialFeedLayout.ts` per the playbook. Process, naming, and the expansion matrix live in [`docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md`](docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md). Scaffold: `npm run new-cta-frame -- --id=my_frame_v1` from `packages/generation`.

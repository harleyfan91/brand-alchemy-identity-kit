...
**Layout budget (folio 05):** When `examples.ctaSurfaces` is non-empty, the PDF stacks **up to two or three** nested surface modules (each may render a full vector shell) **above** the split rail on the Examples spread. There is no fixed “reserved pt” constant; worst-case height is roughly the **sum** of frame subtrees (story/reel families are the tallest media column) plus module chrome and gaps. **Minimum** in-context shells in that path is **0** if all surfaces drop for empty lines after de-dupe (edge case); **design for 1–3** nested shells capped by `contentDensityBias`. Authoritative detail, footprint references, and mitigation options: [`docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md`](docs/guides/CTA_IN_CONTEXT_FRAME_LIBRARY.md#folio-05-layout-budget-brand-identity-guide).

**Slot-model direction (planned):** After CTA copy composition stabilizes, folio 05 should move to a deterministic **slot model**: first classify frames into standardized size classes (`mobile_tall`, `desktop_wide`, `compact_chip`), then select from a small template set (for example `two_mobile`, `mobile_plus_desktop`, `desktop_plus_compact`, `single_desktop`, `single_mobile`) rather than relying on emergent spacing from arbitrary frame heights. The playbook section above is the normative source until code wiring lands.

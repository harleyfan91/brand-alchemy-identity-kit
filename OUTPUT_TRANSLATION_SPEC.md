...
**Distinct copy across folio 05 surfaces:** When multiple surfaces render in one kit (e.g. Instagram feed + Facebook story via `social` + `social_secondary`), tuple selection rotates through the prescriptive bank so the **rendered** line pair on the second surface differs from the first when the bank has another valid variant (`composeSurfaceCtaLines` / `usedRenderedCtaSignatures`). Frames and platform labels still differ; copy should not repeat verbatim across the only two CTA modules unless the bank has a single tuple after dedupe against voice/sample lines.
